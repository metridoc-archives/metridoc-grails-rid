 ...
 */
@TestFor(RidAdminModeOfConsultationController)
@Mock(RidModeOfConsultation)
class RidAdminModeOfConsultationControllerTests {

    void testInvalidSave() {
        def token = SynchronizerTokensHolder.store(session)
        controller.params[SynchronizerTokensHolder.TOKEN_KEY] = token.generateToken("/ridAdminModeOfConsultation/list")
        controller.params[SynchronizerTokensHolder.TOKEN_URI] = "/ridAdminModeOfConsultation/list"

        controller.params.inForm = 1
        controller.save()

        assert response.redirectedUrl == '/ridAdminModeOfConsultation/list'
